<?php
/**
 * WesSciJo — Archival Broadsheet
 * Concept theme for The Wesleyan Science Journal.
 */

function wessci_broadsheet_setup() {
...
add_action( 'after_setup_theme', 'wessci_broadsheet_setup' );

function wessci_broadsheet_assets() {
	wp_enqueue_style(
		'wessci-fonts',
		'https://fonts.googleapis.com/css2?family=Libre+Caslon+Display&family=Libre+Caslon+Text:wght@400;700&family=Libre+Franklin:wght@400;500;600;700&display=swap',
		array(),
		null
	);
	wp_enqueue_style( 'wessci-broadsheet', get_stylesheet_uri(), array( 'wessci-fonts' ), '1.0.0' );
}
add_action( 'wp_enqueue_scripts', 'wessci_broadsheet_assets' );
